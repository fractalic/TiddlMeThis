...
for blob in data:
    ...
    with open(group_dir + blob['meta']['title']+'.md', 'wb') as doc_out:
        doc_out.write(doc_file)

    with open(group_dir + blob['meta']['title']+'.md.meta', 'w') as doc_meta:
        for prop in blob['meta']:
            doc_meta.write(prop + ': ' + blob['meta'][prop])
            doc_meta.write("\n") # we expect python to convert to os line-ending

    with open(group_dir + '_importables', 'w') as importables_list:
        importables_list.truncate()
        importables_list.write(blob['meta']['title']+'.md'+'\n')
